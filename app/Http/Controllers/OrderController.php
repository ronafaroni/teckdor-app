...
class OrderController extends Controller
{
    ...
    public function orderPayment()
    {
        ...
        $orders = Order::with('product.supplier', 'user')
            ...
            ->orderBy('updated_at', 'desc')
            ->get()
            ->groupBy('product.supplier.name_supplier'); // Kelompokkan berdasarkan nama supplier

        // Paginasi manual
        $perPage = 1; // Jumlah data per halaman
        $currentPage = Paginator::resolveCurrentPage('page'); // Ambil nomor halaman saat ini
        $items = $orders->slice(($currentPage - 1) * $perPage, $perPage)->all(); // Ambil data untuk halaman saat ini
        $paginatedOrders = new LengthAwarePaginator($items, $orders->count(), $perPage, $currentPage, [
            'path' => Paginator::resolveCurrentPath(), // Path untuk pagination
        ]);

        return view('order.order-payment', compact('paginatedOrders'));
    }
    ...
    public function orderApprove(Request $request, $id)
    {
        ...
        $order->order_date = now('Asia/Jakarta')->toDateString();
        $order->save();
    ...
    public function searchLog(Request $request)
    {
        ...
        // Query pencarian berdasarkan rentang tanggal dan waktu
        $orders = Order::with('product.supplier', 'user')
            ->where('is_draft', 'false')
            ->whereBetween('order_date', [$date_awal, $date_akhir])
            ->orderBy('updated_at', 'desc')
            ->get()
            ->groupBy('code_order');
