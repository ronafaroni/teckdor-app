...
@section('finance-content')
    <div class="d-flex flex-wrap align-items-center justify-content-between gap-3 mb-24">
        <h6 class="fw-semibold mb-0">Payment Supplier List</h6>
                ...
                <tbody align="center">
                    @foreach ($payment as $itemPayment)
                        <tr>
                            ...
                            <td>{{ $itemPayment->product->supplier->name_supplier }}</td>
                            <td>{{ $itemPayment->code_order }}</td>
                            <td>{{ $itemPayment->product->name_product }}</td>
                            <td>{{ $itemPayment->user->name }}</td>
                            {{-- <td>{{ 'Rp. ' . number_format($itemPayment->total_payment * $itemPayment->qty) }}</td> --}}
                            <td>
                                @if ($itemPayment->supplier_payment_status == 0)
                            ...
                            <td>
                                <a href="{{ route('payment-supplier-detail', $itemPayment->product_id) }}"
                                    class="btn btn-sm btn-dark radius-8 d-flex align-items-center gap-2">
                                    <iconify-icon icon="mdi:payment-settings" class="menu-icon text-lg"></iconify-icon>
                                    <span>Payment</span>
                                </a>
                            </td>
                        </tr>
